...
def apply(fct, sample, sample_keys, frame_idcs, scales):
    for sample_key in sample_keys:
        for frame_idx in frame_idcs:
            for scale in scales:
                if (sample_key, frame_idx, scale) in sample:
                    sample[(sample_key, frame_idx, scale)] = fct(sample[(sample_key, frame_idx, scale)])
...
class RandomColorJitter:
    ...
    def __init__(self, brightness, contrast, saturation, hue, p, sample_keys, temp_context, scales):
    ...
    def __call__(self, sample):
        ...
        if torch.rand(1) < self.p:
            if sample["weather"] != 'day-rain' and sample["weather"] != 'night-clear' and sample["weather"] != 'night-rain':
                # print(sample["weather"])
                sample = apply(color_jitter, sample, self.sample_keys, self.temp_context, self.scales)

        return sample


class RandomGaussianNoise:
    def __init__(self, min_rand, max_rand, p, sample_size, sample_keys, temp_context, scales):
                ...
                for frame_idx in self.temp_context:
                    ...
                    for sample_key in self.sample_keys:
                        for scale in self.scales:
                            if (sample_key, frame_idx, scale) in sample:
                                width, height = sample[(sample_key, frame_idx, scale)].size
                                noise = torch.randn((3, height, width))
                                sample[(sample_key, frame_idx, scale)] = gaussian_noise(sample[(sample_key, frame_idx, scale)], noise)

        return sample


class RandomGaussianBlur:
    def __init__(self, kernel_size, sigma, p, sample_keys, temp_context, scales):
        self.p = p
        self.sample_keys = sample_keys
        self.temp_context = temp_context
    ...
    def __call__(self, sample):
        if torch.rand(1) < self.p:
            sample = apply(self.gaussian_blur, sample, self.sample_keys, self.temp_context, self.scales)
...
class ToTensor:
    def __init__(self, sample_keys, temp_context, scales):
    ...
    def __call__(self, sample):
        ...
        sample = apply(self.to_tensor, sample, [self.sample_keys[0]], self.temp_context, self.scales)
        sample = apply(self.to_tensor, sample, self.sample_keys[1:3], self.temp_context, [0])
        sample = apply(self.to_tensor, sample, [self.sample_keys[-1]], self.temp_context, [0])
        for i in [0, -1, 1]:
            del sample[("color", i, -1)]
            del sample[("color_aug", i, -1)]
            del sample[("color_aug_pose", i, -1)]
            del sample[("color_real", i, -1)]
...
class NormalizeDynamic_Maxpixel:
    def __init__(self, cfg):
        self.normalize_mode = cfg.DATASET.AUGMENTATION.NORMALIZE.MODE
        assert self.normalize_mode in ['Dataset', 'Daytime', 'Image']
        if self.normalize_mode == 'Dataset':
...
class DaytimeTranslation:
    def __init__(self, path, direction, p, sample_keys, temp_context, scales, resized_width, resized_height):
        ...
        self.res_width = resized_width
        self.res_height = resized_height
    ...
    def __call__(self, sample):
        if torch.rand(1) < self.p:
            if self.direction == 'night->day' and 'night' in sample['weather'] or \
                    self.direction == 'day->night' and 'day-clear' in sample['weather'] or \
                ...
                for sample_key in self.sample_keys:
                    for temp in self.temp_context:
                        for i in range(self.num_scales):
                            img = Image.open(os.path.join(self.path, sample[('filename', temp)]))
                            sample[(sample_key, temp, i)] = self.resize[i](img)
