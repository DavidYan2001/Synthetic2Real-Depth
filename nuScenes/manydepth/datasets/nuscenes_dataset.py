...
class NuScenesDataset:
    def __init__(self, cfg, transform, dataset_version):
        ...
        self.dataroot = os.path.join(cfg.data_path, self.dataset_version)
        self.drop_static = cfg.drop_static
        ...
        self.camera_sensors = [self.orientation_to_camera[orientation] for orientation in cfg.orientations]
        ...
        self.original_width = cfg.original_width
        self.original_height = cfg.original_height
        self.res_width = cfg.resized_width
        self.res_height = cfg.resized_height
        self.scales = cfg.scales
        self.temp_context = cfg.temp_context
        self.weather_conditions_train = cfg.weather_train
        self.weather_conditions_train_real = cfg.weather_train_real
        self.weather_conditions_val = cfg.weather_val
        self.weather_conditions_eval = cfg.weather_test
        ...
        for i in range(self.num_scales):
            s = 2 ** i
            self.resize[i] = transforms.Resize((self.res_height // s, self.res_width // s),
                                               interpolation=self.interp)

        self.load_depth_gt = cfg.load_gt_depth
        self.load_pose_gt = cfg.load_gt_pose
        self.load_color_full_size = cfg.color_full_size

        if dataset_version in ['trainval', 'mini']:
            self.scenes_weather_info = self.get_scenes_weather_info()
            self.train_idcs, self.val_idcs, self.train_real_idcs = self.get_train_val_indices()
    ...
    def getitem(self, index, mode):
        ...
        real_idx = random.randrange(start=0, stop=len(self.train_real_idcs), step=1)
        real_idx = self.train_real_idcs[real_idx]
        temp_context_real = [0,-1,1]
        (frame_idx_real, camera_sensor_real) = self.sample_to_sensor_sample_data[real_idx]
        weather_real = self.get_weather(real_idx)
        ...
        for temp in temp_context:
            ...
            for sample_key in color_keys:
                sample[(sample_key, temp, -1)] = img.copy()
                if sample_key=='color':
                    for i in range(self.num_scales):
                        sample[(sample_key, temp, i)] = self.resize[i](sample[(sample_key, temp, i - 1)])
                if sample_key=='color_aug' or sample_key=='color_aug_pose':
                    sample[(sample_key, temp, 0)] = self.resize[0](sample[(sample_key, temp, -1)])

        for temp in temp_context_real:
            img, filepath = self.get_color(frame_idx_real, camera_sensor_real, temp)
            sample[('filename_real', temp)] = filepath
            for sample_key in ['color_real']:
                sample[(sample_key, temp, -1)] = img.copy()
                if sample_key=='color_real':
                    # for i in range(self.num_scales):
                    sample[(sample_key, temp, 0)] = self.resize[0](sample[(sample_key, temp, -1)])
                # if sample_key=='color_aug' or sample_key=='color_aug_pose':
                #     sample[(sample_key, temp, 0)] = self.resize[0](sample[(sample_key, temp, -1)])
        ...
        K = np.identity(4)
        K[:3,:3] = self.get_cam_intrinsics(camera_sensor)
        ...
        for scale in range(self.num_scales+2):
            Ks = K.copy()
            # print(K)
            Ks[0,:] /= self.original_width
            Ks[0, :] *= self.res_width  // (2 ** scale)
            Ks[1, :] /= self.original_height
            Ks[1, :] *= self.res_height  // (2 ** scale)

            inv_Ks = np.linalg.pinv(Ks)

            sample[("K", scale)] = torch.from_numpy(Ks).float()
            sample[("inv_K", scale)] = torch.from_numpy(inv_Ks).float()

        # print(sample[("K", 2)])
        # print(sample[("K", 3)])

        #load for intrinsics of real data
        K = np.identity(4)
        K[:3,:3] = self.get_cam_intrinsics(camera_sensor_real)

        # sample['K'] = torch.Tensor(K.copy())
        if self.load_color_full_size:
            sample['K_original_real'] = torch.Tensor(K.copy())
        ...
        for scale in range(self.num_scales+2):
            ...
            Ks[1, :] *= self.res_height  // (2 ** scale)
            inv_Ks = np.linalg.pinv(Ks)
            sample[("K_real", scale)] = torch.from_numpy(Ks).float()
            sample[("inv_K_real", scale)] = torch.from_numpy(inv_Ks).float()
    ...
    def get_train_val_indices(self):
        ...
        scenes_split_tokens_map = {'train': [], 'val': [], 'train_real':[]}
        weather_counter = {
        ...
        for scene in self.nusc.scene:
            ...
            if scene['name'] in scenes_split_names_map['train'] and weather_curr in self.weather_conditions_train_real:
                scenes_split_tokens_map['train_real'].append(scene_token)
        ...
        train_sample_indices, val_samples_indices, train_real_sample_indices = self.split_train_val_temp_valid_samples(scenes_split_tokens_map)

        self.console_logger.info(f"Weather distribution in train and val scenes:\n{weather_counter}")

        return np.asarray(train_sample_indices), np.asarray(val_samples_indices), np.asarray(train_real_sample_indices)
    ...
    def split_train_val_temp_valid_samples(self, scenes_split_lut):
        ...
        train_real_sample_indices = []
        val_sample_indices = []
        for i, sample in enumerate(self.nusc.sample):
            scene_token = sample['scene_token']
            if scene_token in scenes_split_lut['train_real']:
                self.add_index_to_index_list(i, train_real_sample_indices)
        ...
        return train_sample_indices, val_sample_indices, train_real_sample_indices
