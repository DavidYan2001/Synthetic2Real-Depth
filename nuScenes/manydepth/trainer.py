...
import os
# os.environ["MKL_NUM_THREADS"] = "2"  # noqa F402
# os.environ["NUMEXPR_NUM_THREADS"] = "2"  # noqa F402
# os.environ["OMP_NUM_THREADS"] = "2"  # noqa F402
...
from nuScenes.manydepth import datasets, networks
from nuScenes.manydepth.datasets.nuscenes_dataset import NuScenesDataset, Subset, NuScenesDataSubset
import matplotlib.pyplot as plt
from nuScenes.manydepth.utils import compute_depth_metrics
import copy
from .visualize import Visualizer
# from diffusers import UNet2DModel, DDPMScheduler, DDIMScheduler
from .config_util import recursive_load_config
...
class Trainer:
    def __init__(self, options):
        ...
        self.models["teacher_pose_encoder"] = \
            networks.ResnetEncoder(18, self.opt.weights_init == "pretrained",
                                   num_input_images=self.num_pose_frames, normalize_mode='Daytime')
        ...
        #here we do not train DDPM model but train depth model
        ...
        # file_path = "/mnt/weilong/manydepth4all_high_level_cv/depth_stat_day_clear.txt"
        file_path = "/mnt/codemnt/weilong/syn2real_depth/nuScenes/depth_stat_day_clear_diff_origin_256bins.txt"
        self.distribution_daytime = read_txt_to_tensor(file_path).to(self.device).view(257)
        ...
        to_tensor = datasets.transforms.ToTensor(sample_keys=['color', 'color_aug', 'color_aug_pose','color_real'],
        ...
        transform = {'train': datasets.transforms.CustomCompose(train_transform),
                     'train_real': datasets.transforms.CustomCompose(real_transform),
                     'val': datasets.transforms.CustomCompose(val_transform),
                     'eval': datasets.transforms.CustomCompose(eval_test_transform)}
        
        if self.opt.running_mode == "train":
            nuscenes_dataset = NuScenesDataset(self.opt, transform, 'trainval')
            ...
            # self.valid_sampler = DistributedSampler(self.val_dataset)
    ...
    def _replace_unet_conv_in(self):
        ...
        _weight = self.models["DDPMmodel"].unet.conv_in.weight.clone()  # [320, 4, 3, 3]
        _bias = self.models["DDPMmodel"].unet.conv_in.bias.clone()  # [320]
        _weight = _weight.repeat((1, 2, 1, 1))  # Keep selected channel(s)
        # half the activation magnitude
        _weight *= 0.5
        # new conv_in channel
        _n_convin_out_channel = self.models["DDPMmodel"].unet.conv_in.out_channels
        _new_conv_in = Conv2d(
            8, _n_convin_out_channel, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)
        )
        _new_conv_in.weight = Parameter(_weight)
        _new_conv_in.bias = Parameter(_bias)
        self.models["DDPMmodel"].unet.conv_in = _new_conv_in
        print("Unet conv_in layer is replaced")
        # replace config
        self.models["DDPMmodel"].unet.config["in_channels"] = 8
    ...
    def visualize_depth(self, inv_depth, filename):
        from manydepth.utils import viz_inv_depth
        import cv2
        inv_depth = cv2.cvtColor(viz_inv_depth(inv_depth, percentile=95).astype(np.float32) * 255, cv2.COLOR_RGB2BGR)
        cv2.imwrite( f"{filename}_inv_depth.jpg", inv_depth)

    def visualize_consistency(self, inv_depth, filename):
        from manydepth.utils import viz_inv_depth
        import cv2
        inv_depth = cv2.cvtColor(viz_inv_depth(inv_depth, percentile=95, colormap = 'viridis').astype(np.float32) * 255, cv2.COLOR_RGB2BGR )
        cv2.imwrite( f"{filename}_consistency.jpg", inv_depth)
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            # print("cv_loss={}".format(losses["cv_loss"]))
    ...
    def epoch_val(self, val_teacher=False, val_multi=False):
        ...
        while True:
            ...
            with torch.no_grad():
                ...
                # outputs, mse_loss = self.process_batch_val_DDIM(inputs, val_seed_ls)
                outputs = self.process_batch_val(inputs, val_multi=val_multi, val_teacher=val_teacher)
                depth = outputs["depth"]
    ...
    def process_batch(self, inputs, is_train=False):
        ...
        #teacher+daytime model (pose+depth)
        with torch.no_grad():
            #daytime model path
            ...
            daytime_features, daytime_lowest_cost, daytime_confidence_mask, daytime_cost_volume, daytime_full_cv = self.models["daytime_encoder"](inputs["color", 0, 0],
            ...
            teacher_features, teacher_lowest_cost, teacher_confidence_mask, teacher_cost_volume, teacher_full_cv = self.models["teacher_encoder"](inputs["color", 0, 0],
        ...
        frame_augmentation_mask = torch.zeros([batch_size, 1, 1, 1]).to(self.device).float()
        weather_mask = torch.zeros([batch_size, 1, 1, 1]).to(self.device).float()
        if is_train and not self.opt.no_matching_augmentation:
            for batch_idx in range(batch_size):
                ...
                if rand_num < 0.25:
                    ...
                    frame_augmentation_mask[batch_idx] += 1
        ...
        features, lowest_cost, confidence_mask, cost_volume, full_cv = self.models["encoder"](inputs["color_aug", 0, 0],
                                                                        lookup_frames,
        ...
        losses = self.compute_TS_losses(outputs, teacher_outputs, daytime_model_multi_disps, cost_volume, teacher_cost_volume, augmentation_mask, weather_mask, confidence_mask, teacher_confidence_mask, full_cv, teacher_full_cv, frame_augmentation_mask)
        ...
        distribution_student = self.differentiable_histogram(depth, num_bins=257, min_depth=3.5, max_depth=80)

        kl_div = 0.001 * F.kl_div(distribution_student.log(), self.distribution_daytime, reduction='mean')
    ...
    def differentiable_histogram(self, img, num_bins, min_depth, max_depth):
        ...
        L = (max_depth - min_depth) / (num_bins-1)  # Bin width
        width = L/2
        bin_centers = torch.linspace(min_depth, max_depth, num_bins).to(self.device)
    ...
    def loss_KL_div(self, tensor_a, tensor_b, reduction='mean'):
    ...
    def process_batch_val(self, inputs, is_train=False, val_multi=False, val_teacher=False):
        ...
        # multi frame path
            ...
            features, lowest_cost, confidence_mask, costvolume, full_cv = self.models["encoder"](inputs["color_aug", 0, 0],
                                                                            lookup_frames,
                                                                            inputs['weather_depth'],
                                                                            relative_poses,
                                                                            inputs[('K', 2)],
            ...
            features, lowest_cost_gun, confidence_mask, cost_volume,full_cv = self.models["encoder"](inputs["color_aug", 0, 0],
                                                                            lookup_frames,
                                                                            inputs['weather_depth'],
                                                                            relative_poses,
                                                                            inputs[('K', 2)],
            ...
            with torch.no_grad():
                ...
                for fi in self.matching_ids[1:]:
                    ...
                    for batch_idx, feat in enumerate(pose_feats[fi]):
                        if feat.sum() == 0:
                            pose[batch_idx] *= 0
                    # print(self.matching_ids)
                    # print(pose)
                    # print(inputs[('pose_gt', -1)])
                    # print(a)
        ...
        with torch.no_grad():
            ...
            for fi in self.matching_ids[1:]:
                if fi < 0:
                    ...
                    # # now find 0->fi pose
                    # if fi != -1:
                    #     pose = torch.matmul(pose, inputs[('teacher_relative_pose', fi + 1)])
    ...
    def predict_poses_real(self, inputs, features=None):
        """Predict poses between input frames for monocular sequences.
        """
        outputs = {}
        
            # now we need poses for matching - compute without gradients
        pose_feats = {f_i: inputs["color_real", f_i, 0] for f_i in self.matching_ids}
        # with torch.no_grad():
        # compute pose from 0->-1, -1->-2, -2->-3 etc and multiply to find 0->-3
        for fi in self.matching_ids[1:]:
            if fi < 0:
                if self.opt.maintain_temp:
                    ...
                    pose_inputs = [pose_feats[fi + 1], pose_feats[fi]]
                pose_inputs = [self.models["teacher_pose_encoder"](torch.cat(pose_inputs, 1),inputs['weather_real'])]
                axisangle, translation = self.models["teacher_pose"](pose_inputs)
    ...
    def compute_TS_losses(self, outputs, teacher_outputs, daytime_model_disps, cost_volume, teacher_cost_volume, augmentation_mask,weather_mask, confidence_mask, teacher_confidence_mask, student_full_cv, teacher_full_cv, frame_augmentation_mask):
        ...
        for fi in self.matching_ids[1:]:
            ...
            mask = torch.ones_like(teacher_axisangle)
            # print(mask)
            axisangle *= mask
            teacher_axisangle *= mask
            translation *= mask
            teacher_translation *= mask

            trans_loss = (torch.abs(translation-teacher_translation)**2).sum()/(mask.sum())
            rot_loss = (torch.abs(axisangle-teacher_axisangle)**2).sum()/(mask.sum())
        ...
        for scale in self.opt.scales:
            ...
            confidence_map = torch.exp(-1.0*torch.abs((teacher_disp-daytime_model_disp)/teacher_disp))+1.0

            mask = torch.ones_like(disp)
            consistency_loss = (mask*confidence_map*torch.abs((disp - teacher_disp)/disp)).sum()/(mask.sum())
            losses['consistency_loss/{}'.format(scale)] = consistency_loss
            total_loss = total_loss + consistency_loss/len(self.opt.scales)


        cv_loss = self.compute_L1loss_with_mask(cost_volume,teacher_cost_volume, confidence_mask, augmentation_mask, frame_augmentation_mask, weather_mask,teacher_confidence_mask, student_full_cv, teacher_full_cv)
        
        losses['cv_loss'] = cv_loss
    ...
    def compute_L1loss_with_mask(self, cost_volume, teacher_cost_volume, mask, augmentation_mask, frame_augmentation_mask, weather_mask, teacher_mask, student_full_cv, teacher_full_cv, dim=1):
        ...
        L1loss = cv_diff.sum()/(aug_mask.sum()+1e-7)
    ...
    def save_model(self, save_step=False):
        ...
        for model_name, model in self.models.items():
            ...
            if model_name =='DDPMmodel':
                model_name='unet'
                save_path = os.path.join(save_folder, "{}.pth".format(model_name))
                to_save = model.unet.state_dict()
            else:
    ...
    def load_model(self):
        ...
        for n in self.opt.models_to_load:
            if n == "unet":
                ...
                model_dict = self.models["DDPMmodel"].unet.state_dict()
                pretrained_dict = torch.load(path)

                pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
                model_dict.update(pretrained_dict)
                self.models["DDPMmodel"].unet.load_state_dict(model_dict)
