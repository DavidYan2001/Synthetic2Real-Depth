...
class RobotCarDataset:
    def __init__(self, cfg, transform):
        ...
        self.dataroot = cfg.data_path
        self.weather = {'train': cfg.weather_train, 'val': cfg.weather_val, 'test': cfg.weather_val}
        # print(self.weather)
        ...
        self.load_split_files_path = '/mnt/datamnt/weilong/datasets/robust_depth/robotcar/splits'
        self.load_filter_files_path = '/mnt/datamnt/weilong/datasets/robust_depth/robotcar/filter'
        self.point_dim = 3
        ...
        # print(self.weather_conditions_train)
        self.weather_conditions_val = cfg.weather_val
        self.weather_conditions_eval = cfg.weather_val
        ...
        self.u, self.v = cfg.crop_top, cfg.crop_left
        self.crop = partial(transforms.functional.crop, top=cfg.crop_top, left=cfg.crop_left, height=cfg.crop_height, width=cfg.crop_width)
        ...
        def read_and_unpack_split_file(split_file, index, mode, timestamp_filter_list=None):
            ...
            # for i, line in enumerate(split_file):
                ...
                # next_timestamp = split_file[i+1].split(' ')
                # color = {-1: timestamp[0].rstrip(), 0: timestamp[1].rstrip(), 1: timestamp[2].rstrip()} if mode != 'test' else {0: timestamp[0]}
                color = {-1: timestamp[0].rstrip(), 0: timestamp[1].rstrip(), 1: timestamp[2].rstrip()} if mode != 'test' else { 0: timestamp[0].rstrip() }
    ...
    def get_color(self, frame_idx, camera_sensor, temp):
        # print(f"{os.path.join(self.sample_data[frame_idx]['scene'], camera_sensor, self.sample_data[frame_idx]['color'][temp])}.png")
        # print(self.sample_data[frame_idx])
    ...
    def getitem(self, index, mode):
        ...
        if self.load_depth_gt and mode in ['val', 'test']:
            ...
            # print(sample['depth_gt'].shape)

        # Get camera intrinsics
        K = np.identity(4)
        K[:3,:3] = self.get_cam_intrinsics(camera_model)
        K[0, 2] -= self.u
        K[1, 2] -= self.v
        ...
        for scale in range(self.num_scales):
            Ks = K.copy()
            # print(K)
            Ks[0,:] /= self.crop_width
            Ks[0, :] *= self.res_width  // (2 ** scale)
            Ks[1, :] /= self.crop_height
