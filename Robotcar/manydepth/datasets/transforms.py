            ...
            if self.direction == 'night->day' and 'night' in sample['weather'] or \
                ...
                for sample_key in self.sample_keys:
                    for temp in self.temp_context:
                        for i in range(self.num_scales):
                            # print(temp)
                            # print(os.path.join(self.path, sample[('filename', temp)]))
                            # file_path = Path(os.path.join(self.path, sample[('filename', temp)]))
                            if os.path.isfile(os.path.join(self.path, sample[('filename', temp)])):
                                img = Image.open(os.path.join(self.path, sample[('filename', temp)]))
                                sample[(sample_key, temp, i)] = self.resize[i](img)
                            else:
                                print("fail to load!!!")
                                img = Image.open(os.path.join(self.path, sample[('filename',0)]))
                                sample[(sample_key, temp, i)] = self.resize[i](img)
