...
from Robotcar.manydepth.layers import BackprojectDepth, Project3D
from ..utils import readlines, sec_to_hm_str, flip_lr
...
class ResnetEncoderMatching(nn.Module):
    ...
    def __init__(self, num_layers, pretrained, input_height, input_width,
                 ...
                 adaptive_bins=False, depth_binning='linear', normalizing_mode='Dataset'):
        ...
        self.backprojector_high_level = BackprojectDepth(batch_size=64,
                                              height=40,
                                              width=68)
        self.projector_high_level = Project3D(batch_size=64,
                                   height=40,
                                   width=68)
        ...
        self.normalizing_mode = normalizing_mode
    ...
    def compute_depth_bins(self, min_depth_bin, max_depth_bin):
        ...
        self.high_level_depth_bins = np.array(
            [np.exp(np.log(3.5) + np.log(80 / 3.5) * i / (64 - 1))
            for i in range(64)])
        
            
        self.high_level_depth_bins = torch.from_numpy(self.high_level_depth_bins).float()

        self.high_level_warp_depths = []
        for depth in self.high_level_depth_bins:
            depth = torch.ones((1, 40, 68)) * depth
            self.high_level_warp_depths.append(depth)
        self.high_level_warp_depths = torch.stack(self.high_level_warp_depths, 0).float()
        if self.is_cuda:
            self.high_level_warp_depths = self.high_level_warp_depths.cuda()
    ...
    def match_features_dotproduct(self, current_feats, lookup_feats, relative_poses, K, invK):
        """Compute a cost volume based on L1 difference between current_feats and lookup_feats.
        ...
        for batch_idx in range(len(current_feats)):
            ...
            for lookup_idx in range(_lookup_feats.shape[1]):
                ...
                #warped: 128*64*80*144
                #current_feats[batch_idx:batch_idx + 1]: 1*64*80*144
                diffs = (warped * current_feats[batch_idx:batch_idx + 1]).mean(
                     1) * edge_mask
    ...
    def match_features_dotproduct_high_level(self, current_feats, lookup_feats, relative_poses, K, invK):
        """Compute a cost volume based on L1 difference between current_feats and lookup_feats.
        ...
        for batch_idx in range(len(current_feats)):
            ...
            world_points = self.backprojector_high_level(self.high_level_warp_depths, _invK)
            ...
            for lookup_idx in range(_lookup_feats.shape[1]):
                ...
                lookup_feat = lookup_feat.repeat([64, 1, 1, 1])
                
                pix_locs = self.projector_high_level(world_points, _K, lookup_pose)
                ...
                # Masking of ref image border
                x_vals = (pix_locs[..., 0].detach() / 2 + 0.5) * (
                    68 - 1)  # convert from (-1, 1) to pixel values
                y_vals = (pix_locs[..., 1].detach() / 2 + 0.5) * (40 - 1)

                edge_mask = (x_vals >= 1.0) * (x_vals <= 40 - 1) * \
                            (y_vals >= 1.0) * (y_vals <= 68 - 1)
                edge_mask = edge_mask.float()

                # masking of current image
                current_mask = torch.zeros_like(edge_mask)
                current_mask[:, 1:-1, 1:-1] = 1.0
                edge_mask = edge_mask * current_mask
    ...
    def match_features_cossim(self, current_feats, lookup_feats, relative_poses, K, invK):
        """Compute a cost volume based on L1 difference between current_feats and lookup_feats.
        ...
        for batch_idx in range(len(current_feats)):
            ...
            for lookup_idx in range(_lookup_feats.shape[1]):
                ...
                #warped: 128*64*80*144
                #current_feats[batch_idx:batch_idx + 1]: 1*64*80*144
                normalized_warped = F.normalize(warped, p=2, dim=1)
                normalized_feat = F.normalize(current_feats[batch_idx:batch_idx + 1], p=2, dim=1)
                diffs = (normalized_warped * normalized_feat).sum(
                     1) * edge_mask
    ...
    def build_cost_volume(self, features_t, features_t_plus_1, search_radius=2):
        """
        构建光流 cost volume，考虑所有边缘像素。
        
        参数:
        - features_t: 当前帧的特征图，形状为 (B, C, H, W)
        - features_t_plus_1: 下一帧的特征图，形状为 (B, C, H, W)
        - search_radius: 搜索窗口的半径 (例如 2 表示 [-2, 2] 范围内进行像素匹配)
        
        返回:
        - cost_volume: 匹配后的 cost volume，形状为 (B, D, H, W)，D 为搜索窗口的大小
        """
        B, C, H, W = features_t.shape
        D = (2 * search_radius + 1) ** 2  # 总的位移方向数量

        # 初始化 cost volume
        cost_volume = torch.zeros(B, D, H, W).to(features_t.device)
        
        # 当前搜索窗口内的所有位移方向
        idx = 0
        for dy in range(-search_radius, search_radius + 1):
            for dx in range(-search_radius, search_radius + 1):
                # 将下一帧的特征图在 dx, dy 方向上平移，考虑边界情况
                shifted_features = F.pad(features_t_plus_1, (search_radius, search_radius, search_radius, search_radius))
                shifted_features = shifted_features[:, :, search_radius+dy:H+search_radius+dy, search_radius+dx:W+search_radius+dx]

                # 计算与当前帧特征的余弦相似度 (或其他相似度度量)
                similarity = (features_t * shifted_features).mean(1) 

                # 将计算出的相似度存入 cost volume
                cost_volume[:, idx, :, :] = similarity
                idx += 1

        return cost_volume

    def feature_extraction(self, image, daytime, return_all_feats=False):
        """ Run feature extraction on an image - first 2 blocks of ResNet"""

        # image = (image - 0.45) / 0.225  # imagenet normalisation
        if self.normalizing_mode=='Dataset':
            x = (image - 0.45) / 0.225
        elif self.normalizing_mode=='Daytime':
            x = torch.zeros_like(image, device=image.device)
            for i in range(len(daytime)):
                if 'day' in daytime[i]:
                    # x[i] = (image[i] - 0.45) / 0.225
                    x[i] = (image[i] - 0.5758) / 0.319
                elif 'night' in daytime[i]:
                    
                    x[i] = (image[i] - 0.2471) / 0.2627
        else:
            assert NameError

        # print(x)
        feats_0 = self.layer0(x)
    ...
    def forward(self, current_image, lookup_images, daytime, poses, K, invK,
                min_depth_bin=None, max_depth_bin=None, inv_poses=None, high_level_K=None, high_lvel_invK=None
                ):

        # feature extraction
        self.features = self.feature_extraction(current_image, daytime, return_all_feats=True)
        current_feats = self.features[-1]


        # feature extraction on lookup images - disable gradients to save memory
        """initial no_grad following"""
        with torch.no_grad():
            ...
            self.ref_feats = self.feature_extraction(lookup_images, daytime,
                                                    return_all_feats=True)
            lookup_feats = self.ref_feats[-1]
            _, chns, height, width = lookup_feats.shape
            current_feats = current_feats.reshape(batch_size, num_frames, chns, height, width)

            invinv_poses = torch.inverse(inv_poses)
            if inv_poses.shape[0] == 9:
                inv_poses[-1,...] *=0
            cost_volume_inv, missing_mask_inv = \
            self.match_features(lookup_feats, current_feats,  inv_poses, K, invK)
            confidence_mask_inv = self.compute_confidence_mask(cost_volume_inv.detach() *
                                                        (1 - missing_mask_inv.detach()))
            cost_volume_inv *= confidence_mask_inv.unsqueeze(1)
            post_matching_feats_inv = self.reduce_conv(torch.cat([self.ref_feats[-1], cost_volume_inv], 1))

            self.ref_feats.append(self.layer2(post_matching_feats_inv))
            self.ref_feats.append(self.layer3(self.ref_feats[-1]))
            self.ref_feats.append(self.layer4(self.ref_feats[-1]))
        """initial no_grad above"""
        ...
        full_cv = cost_volume.clone()

        cost_volume *= confidence_mask.unsqueeze(1)
        # print(cost_volume.requires_grad)
        ...
        lookup_feat = self.ref_feats[-3]
        _, chns, height, width = lookup_feat.shape
        lookup_feat = lookup_feat.reshape(batch_size, num_frames, chns, height, width)

        if invinv_poses.shape[0]==9:
            invinv_poses[-1,...] *= 0


        #build high level depth cost volume
        cost_volume_high_level, missing_mask_high_level = \
            self.match_features_dotproduct_high_level(current_feats=self.features[-3],lookup_feats=lookup_feat,
                                                      relative_poses=invinv_poses,K=high_level_K,
                                                      invK=high_lvel_invK)
        confidence_mask_high_level = self.compute_confidence_mask(cost_volume_high_level.detach() *
                                                        (1 - missing_mask_high_level.detach()))
        
        #build high level optical cost volume
        flow_cv = self.build_cost_volume(self.features[-3], self.ref_feats[-3], search_radius=2)
        viz_flow_cv = torch.argmin(flow_cv,dim=1)
    
       
        return self.features, lowest_cost, confidence_mask_high_level, cost_volume_high_level, full_cv, flow_cv
...
class ResnetEncoder(nn.Module):
    ...
    def __init__(self, num_layers, pretrained, num_input_images=1, normalizing_mode='Dataset', **kwargs):
    ...
    def forward(self, input_image, daytime):
        self.features = []
        if self.normalizing_mode=='Dataset':
            x = (input_image - 0.45) / 0.225
        elif self.normalizing_mode=='Daytime':
            x = torch.zeros_like(input_image, device=input_image.device)
            for i in range(len(daytime)):
                if 'day' in daytime[i]:
                    # x[i] = (image[i] - 0.45) / 0.225
                    x[i] = (input_image[i] - 0.5758) / 0.319
                elif 'night' in daytime[i]:
                    # print(11)
                    x[i] = (input_image[i] - 0.2471) / 0.2627
        else:
            assert NameError
